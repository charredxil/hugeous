    ...
    $.fn.formset = function (opts) {
        ...
        $$ = $(this),
            ...
            updateElementIndex = function (elem, prefix, ndx) {
                ...
                $(elem).find('.input-group-text').text('' + (ndx + 1));
            },
        ...
        $$.each(function (i) {
            ...
            if (hasChildElements(row)) {
                ...
                if (row.is(':visible') && i != $$.length - 1) {
                    insertDeleteLink(row);
                    if (options.disableInitialDels) {
                        row.find('a.' + delCssSelector).addClass("disabled");
                    }
        ...
        if ($$.length) {
            ...
            $$.filter(':last').append('<div class="input-group-append"><a class="' + options.addCssClass + '" href="javascript:void(0)">' + options.addText + '</a></div>');
            addButton = $$.filter(':last').find('a.' + addCssSelector)
            if (hideAddButton) addButton.hide();

            addButton.click(function () {
                var formCount = parseInt(totalForms.val()),
                    row = options.formTemplate.clone(true).removeClass('formset-custom-template'),
                    buttonRow = $(this).parent().parent(),
                    delCssSelector = $.trim(options.deleteCssClass).replace(/\s+/g, '.');
